...
class PreInternQwenQAModel(PreTrainedModel):
    def __init__(self, config):
        ...
        self.language_model = Qwen2ForCausalLM.from_pretrained(config.language_model_path, torch_dtype=config.torch_dtype)
    ...
    def forward(self, images, texts, labels):
        ...
        outputs = self.language_model(inputs_embeds=inputs_embeds, attention_mask=inputs["attention_mask"], labels=inputs["labels"], output_hidden_states=True)
        ...
        img_global_feature = self.pool_features(hidden_states, inputs["input_ids"], img_start_token_id, img_end_token_id).cuda()
        ...
        question_global_feature = self.pool_features(hidden_states, inputs["input_ids"], qus_start_token_id, qus_end_token_id).cuda()
...
def bio_qwen2_5_pre_caption_model_base(**kwargs):
    return PreInternQwenQAModel(BioMiLaConfig())


if __name__ == '__main__':
    from PIL import Image
    selected_entity = "<ENT>Head CT -> Head CT -> Head CT</ENT>, <ENT>Head CT -> Head CT -> Head CT</ENT>"
    text = [f"Relevant matching entity relationships found through the knowledge base: {selected_entity} \n" + f"<QUS>Please describe this image in detail based on the image, about the images.</QUS>",
    ...
    # model(text, label)
    model = PreInternQwenQAModel(BioMiLaConfig()).cuda()
    print(sum(x.numel() for x in model.parameters()))  # 582094696
    ls, ot = model(image, text, label)
    print(ls, ot)
