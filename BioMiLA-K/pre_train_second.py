...
from data import PreQALoRaROCOv2Dataset  # 确保数据集返回 dict 格式的数据，包含 (image, question, answer, image_path)
...
def main(args):
    ...
    # 获取模型构造函数（返回的是模型类）
    model = get_prompt_model_module(model_name)(load_model_path=args.pretrained_first_dir)
    ...
    train_dataset = PreQALoRaROCOv2Dataset(args)

    # 实例化知识库融合模块，并在训练前加载 KG 内部的 MLP 模型（如果保存路径存在）
...
if __name__ == '__main__':
    ...
    # 模型相关参数
    parser.add_argument("--model_select", default="bio_qwen2_5_pre_lora_model", choices=["bio_qwen2_5_pre_lora_model"])
    ...
    parser.add_argument("--grad_accumulation_steps", default=32, type=int)
    ...
    parser.add_argument("--pretrained_first_dir", default="./save/bio_qwen2_5_pre_caption_model_base/checkpoint-2500/model.safetensors")
